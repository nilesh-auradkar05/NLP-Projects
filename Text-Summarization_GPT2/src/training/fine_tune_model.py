import torch
import numpy as np
from src.data.load_gpt2_weights import download_and_load_gpt2
...
class FineTuneModel:
    ...
    def _get_settings_and_params(self):
        settings, params = download_and_load_gpt2(model_size="774M", models_dir="../../model_weights/")
    ...
    def _get_model_config(self):
        ...
        if model_config is not None:
            print(f"Model config for {self.model_name}: {model_config}")
            return model_config
    ...
    def _load_gpt2_weights_into_model(self):
        ...
        for block in range(self.params["n_layers"]):
            ...
            self.model.transformer_blocks[block].attn.W_key.weight = self._assign_params(
